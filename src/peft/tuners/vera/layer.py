...
class VeraLayer(LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        self.lora_A[adapter_name] = nn.Parameter(torch.normal(size=(r, self.in_features), mean=0, std=std_dev), requires_grad=False)
        # Singular values
        # self.lora_E[adapter_name] = nn.Parameter(torch.randn(r, 1))
        self.lora_d[adapter_name] = nn.Parameter(torch.ones(r, 1))
...
class VeraLinear(nn.Module, VeraLayer):
    # VeRA adaptation by a dense layer
    def __init__(
        self,
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        return (
            transpose((self.lora_b[adapter] * self.lora_B[adapter]) @ (self.lora_A[adapter] * self.lora_E[adapter]), self.fan_in_fan_out)
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                x = x.to(lora_A.dtype)
                result += (dropout(x) @ (lora_A * lora_d).T @ (lora_B * lora_b).T) * scaling
